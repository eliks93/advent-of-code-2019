...
const parse1 = input1.replace(/\s/g, '');
const wire1Arr = parse1.split(",")
const parse2 = input2.replace(/\s/g, '');
const wire2Arr = parse2.split(",")
// console.log(wire1Arr)
// console.log(wire2Arr)

let smallWire1 ="R8,U5,L5,D3"
let smallWire2 ="U7,R6,D4,L4"
const x = smallWire1.split(",")
const y = smallWire2.split(",")
...
const plotWire = (wire) => {
  ...
  for(const direction of wire) {
    if(direction[0] === "L") {
      let XlocationSum = path[currentLocation][0];
      let YlocationSum = path[currentLocation][1];

      let stringLocation = "";
      for(let i =1; i < direction.length; i++) {
        stringLocation += direction[i] 
        // console.log(stringLocation)
      }
      XlocationSum -= Number(stringLocation)
      path.push([XlocationSum,
      YlocationSum])
    ...
    if(direction[0] === "U") {
      ...
      YlocationSum += Number(stringLocation)
      path.push([XlocationSum,
      YlocationSum])
      currentLocation = currentLocation + 1
    }
...
console.log(plotWire(x), plotWire(y))
